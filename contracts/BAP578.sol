...
import "./interfaces/IBAP578Staking.sol";
    ...
    UUPSUpgradeable
{
    ...
    /// @dev Staking contract address for transfer notifications
    ...
    function initialize(
        ...
        require(circuitBreakerAddress != address(0), "BAP578: Circuit Breaker address is zero");
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            ...
            // Notify staking contract about ownership transfer
            // Stakes follow the agent to the new owner
            if (stakingContract != address(0)) {
                try IBAP578Staking(stakingContract).onAgentTransfer(tokenId, to) {
                    // Successfully notified staking contract
                } catch {
                    // Staking contract notification failed, but don't block transfer
                    // This can happen if staking contract is paused or upgraded
                }
            }
        }
    }
